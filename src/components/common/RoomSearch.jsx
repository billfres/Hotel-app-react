...
const RoomSearch = () => {
    ...
    const handleSearch = (e) => {
        ...
        if (!checkInMoment.isValid() || !checkOutMoment.isValid()) {
            setErrorMessage(t('Please enter valid dates'));
            return;
        }
        if (!checkOutMoment.isSameOrAfter(checkInMoment)) {
            setErrorMessage(t('Check-out date must be after check-in date.'));
            return;
        }
        setIsLoading(true);
    ...
    return (
        <>
            <Container className="shadow mt-n5 mb-5 py-5">
                    ...
                    <Row className="justify-content-center">
                        ...
                        <Col xs={12} md={3}>
                            <Form.Group controlId="roomType">
                                <Form.Label>{t('Room Type')}</Form.Label>
                                <div className="d-flex">
                                    <RoomTypeSelector
                                        handleRoomInputChange={handleInputChange}
                                        newRoom={searchQuery}
                                    />
                                    <Button variant="secondary" type="submit" className="ml-2 btn-recherche">
                                        {t('Search')}
                                    </Button>
                                </div>
                            </Form.Group>
                ...
                {isLoading ? (
                    <p className="mt-4">{t("Finding available rooms...")}</p>
                ) : availableRooms ? (
                    <RoomSearchResults results={availableRooms} onClearSearch={handleClearSearch} />
                ) : (
                    <p className="mt-4">{t("No rooms available for the selected dates and room type.")}</p>
                )}
